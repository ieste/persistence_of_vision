...
class POVApp(object):
    ...
    def preview_text(self, e):
        font = ImageFont.load("C:\Users\Megan\Documents\ENGG2800\engg2800-team33\Computer\pilfonts\courB14.pil")
        self.text = self.text_entry.get()
        self.d = ImageDraw.Draw(self.img)
        self.text_width, self.text_height = self.d.textsize(self.text)
        print self.text_width, self.text_height
        self.d.text((((self.w/2-((len(self.text)*11)/2)), (self.h/2-(self.text_height)/2)-5)),
    ...
    def preview(self):
        ...
        for y in range(0,96):
            for x in range(0,96):
                r = (32)-(int(round(math.hypot(x-(48), y-(48))-(16),0)))
                if x < (48) and y < (48):
                    theta = 180 - abs(int(round(math.degrees(math.atan((float((48)-x)/((48)-y)))),0))) - (180 - self.w/2)
                    #print ("x:{} y:{} r:{} theta:{}" .format(x, y, r, theta))
                elif x == (48) and y < (48):
                    theta = 180- (180 - self.w/2)
                elif x < (48) and y == (48):
                    theta = 90- (180 - self.w/2)
                elif x == (48) and y > (48):
                    theta = 0- (180 - self.w/2)
                elif x > (48) and y == (48):
                    theta = 270- (180 - self.w/2)
                elif x < (48) and y > (48):
                    theta = abs(int(round(math.degrees(math.atan((float((48)-x)/(y-(48))))),0)))- (180 - self.w/2)
                    #print ("x:{} y:{} r:{} theta:{}" .format(x, y, r, theta))
                elif x > (48) and y < (48):
                    theta = (abs(int(round(math.degrees(math.atan((float((48)-x)/((48)-y)))),0))) + 180)- (180 - self.w/2)
                elif x > (48) and y > (48):
                    theta = 360 -abs(int(round(math.degrees(math.atan((float((48)-x)/((48)-y)))),0)))- (180 - self.w/2)
                if r<(32) and r>0:
